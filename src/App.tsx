...
function RuntimeSetupGuide({ detail }: { detail: string }) {
  return <div className="runtime-preflight"><code>{detail}</code><ol><li>Install Ruby 4.0.6 with rbenv or Homebrew.</li><li>Using that Ruby, run <code>gem install rubyn-code</code>.</li><li>Quit and reopen Rubyn Harness.</li></ol></div>;
}
...
function ProjectRequired() {
  ...
  if (engineState === "unavailable") {
    return (
      <EmptyState
        title="Finish Rubyn runtime setup"
        action={<div className="runtime-actions"><RuntimeSetupGuide detail={engineDetail} /><button className="button primary" onClick={() => setView("projects")}>Open project runtime <ArrowRight size={15} /></button></div>}
      >
        Rubyn Code cannot start yet. Set up a supported Ruby runtime before choosing a repository or connecting model-backed work.
      </EmptyState>
    );
  }
  return (
    ...
    <section>
      ...
      <div className="chat-layout conversation-shell">
        {selected ? <article className="chat-thread">
          <header><div className="conversation-heading"><div className="conversation-title"><span className={`pulse ${statusOf(selected)}`} /><strong>{conversationTitle(selected, 80)}</strong><small>Conversation · {runLabel(selected)}{selected.background ? " · background task" : ""}</small></div><RunUsageSummary usage={tokenUsage} compact /></div><div className="conversation-actions">{turnActive ? <button className="button danger compact" onClick={() => void cancel(selected)}><CircleStop size={14} />Stop turn</button> : selected.running ? <button className="button primary compact" onClick={() => void cancel(selected)}><Check size={14} />Finish conversation</button> : <button className="button quiet compact" onClick={() => selectRun(selected.id)}><ShieldCheck size={14} />Review changes</button>}</div></header>
    ...
    <section>
      ...
      <div className="project-layout">
        ...
        <article className="settings-card">
          ...
          {engineState === "unavailable" && <RuntimeSetupGuide detail={engineDetail} />}
