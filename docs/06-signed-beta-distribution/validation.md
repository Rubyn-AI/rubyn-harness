...
A Developer ID-signed build completed at `src-tauri/target/universal-apple-darwin/release/bundle`. Its executable contains both x86_64 and arm64 slices, `LSMinimumSystemVersion` is 13.0, and the bundle version is 0.1.15. Both the app and DMG satisfy their designated requirements under `Developer ID Application: Matthew Suttles (8MWPMGK3YN)`, and the app signature carries the hardened-runtime flag.
